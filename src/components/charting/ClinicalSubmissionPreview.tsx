...
import { AlertCircle, FileText, ArrowRight, ShieldCheck, ShieldAlert } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full bg-slate-100/50 p-4 md:p-8 overflow-y-auto custom-scrollbar">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h3 className={`text-sm font-black uppercase tracking-wide ${isPassing ? 'text-green-800' : 'text-slate-800'}`}>
                        {isPassing ? 'CLEARINGHOUSE READY' : 'DRAFT IN PROGRESS'}
                    </h3>
                    <p className={`text-xs ${isPassing ? 'text-green-600' : 'text-slate-500'}`}>
                        {isPassing 
                            ? 'Documentation meets all standard requirements.' 
                            : `${missingCount} required item${missingCount !== 1 ? 's' : ''} missing for claim approval.`}
                    </p>
                </div>
            </div>
            {!isPassing && (
                <div className="text-right">
                    <span className="text-2xl font-black text-slate-400">{Math.floor(completeness.percent)}%</span>
                    <div className="text-[10px] font-bold text-slate-300 uppercase tracking-wider">Ready</div>
                </div>
            )}
        </div>
        ...
        <div className="px-12 py-8 border-t border-slate-100 text-[10px] text-slate-300 flex justify-between items-center mt-auto">
            <span>Generated by DentalPlan Pro Claim Studio</span>
