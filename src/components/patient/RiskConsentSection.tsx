...
export const RiskConsentSection: React.FC<RiskConsentSectionProps> = ({ assignedRisks = [] }) => {
  const displayRisks = assignedRisks.filter(r => r.isActive).sort((a, b) => a.sortOrder - b.sortOrder);
    ...
    <section className="py-12 md:py-16 px-4 md:px-6 bg-white border-t border-gray-100">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mt-8 bg-gray-50 p-4 rounded-xl flex gap-4 text-sm text-gray-600 border border-gray-100">
            ...
            <div>
                <p>Please review these items carefully. By proceeding with treatment, you acknowledge that these risks have been explained to you and you have had the opportunity to ask questions.</p>
                <p className="text-xs text-gray-400 mt-2 italic">The risks listed above include common and clinically relevant possibilities, but they are not exhaustive. Other rare or unforeseen complications may occur. Your dentist will explain any additional concerns specific to your case.</p>
            </div>
        </div>
      </div>
    </section>
