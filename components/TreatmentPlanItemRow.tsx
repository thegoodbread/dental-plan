...
export const TreatmentPlanItemRow: React.FC<TreatmentPlanItemRowProps> = ({ item, onUpdate, onDelete }) => {
  ...
  const [teethStr, setTeethStr] = useState(item.selectedTeeth?.join(', ') || '');
  const [baseFee, setBaseFee] = useState(item.baseFee);
  const [discount, setDiscount] = useState(item.discount);

  const handleSave = () => {
    // Parse teeth string back to array
    const teeth = teethStr.split(',')
      .map(t => t.trim())
      .filter(t => t.length > 0);
  ...
  const handleCancel = () => {
    setTeethStr(item.selectedTeeth?.join(', ') || '');
      ...
      if (isEditing) {
        return (
           <input 
             ...
             placeholder="e.g. 1, 3, 16"
             value={teethStr}
             onChange={e => setTeethStr(e.target.value)}
           />
        );
      }
      return <span className="text-gray-900">{item.selectedTeeth?.join(', ') || 'Select teeth...'}</span>;
    }
  ...
  return (
    <tr className={`border-b border-gray-100 last:border-0 hover:bg-gray-50 group transition-colors ${isEditing ? 'bg-blue-50/50' : ''}`}>
      {/* Procedure */}
      ...
      {/* Net Fee */}
      <td className="px-4 py-3 text-right text-sm font-bold text-gray-900 bg-gray-50/50">
